...
const EndDaySelecter = ({
  ...
  const { endDate: endDay, startDate: startDay } = filterFormState;
  return (
    <div className="flex-1 flex max-w-[38.6rem] min-w-[20rem] w-[28rem] flex-col gap-[1.2rem] relative">
      <div className="relative w-full">
        <DatePicker
          locale={ko}
          //   disabled={!startDay}
          selected={!endDay ? new Date() : endDay}
          minDate={startDay}
          onChange={(date) => {
            if (!date) return;

            setFilterDateFormState('endDate', date);
          }}
          ...
          customInput={
            ...
            >
              {!endDay ? '날짜를 선택해주세요' : formatDatePicker(endDay)}
            </Chip>
          }
